          ...
          <div class="card p-3 mt-3">
            ...
            <div class="row">
              ...
              if (is_null($idkategori) || (!$idkategori)) {
                $data = mysqli_query($db, "SELECT * FROM berita b JOIN kategori k ON b.id_kategori = k.id_kategori JOIN penulis p ON b.id_penulis = p.id_penulis ORDER BY created_at DESC;");
                $jumlah_data = mysqli_num_rows($data);
                $total_halaman = ceil($jumlah_data / $batas);

                $databerita = mysqli_query($db, "SELECT * FROM berita b JOIN kategori k ON b.id_kategori = k.id_kategori JOIN penulis p ON b.id_penulis = p.id_penulis ORDER BY created_at DESC LIMIT $halaman_awal, $batas");
              } else {
                $data = mysqli_query($db, "SELECT * FROM berita b JOIN kategori k ON b.id_kategori = k.id_kategori JOIN penulis p ON b.id_penulis = p.id_penulis WHERE b.id_kategori=$idkategori ORDER BY created_at DESC;");
                $jumlah_data = mysqli_num_rows($data);
                $total_halaman = ceil($jumlah_data / $batas);

                $databerita = mysqli_query($db, "SELECT * FROM berita b JOIN kategori k ON b.id_kategori = k.id_kategori JOIN penulis p ON b.id_penulis = p.id_penulis WHERE b.id_kategori=$idkategori ORDER BY created_at DESC LIMIT $halaman_awal, $batas");
              }
              ...
              ?>
                ...
                <div class="col-md-6">
                  <div class="card mb-4">
                    <div class="container">
                      <h5 class="card-title mt-3"><?= $row['judul_berita']; ?>
                      </h5>
                      <p class="text-muted"><?= $tanggal; ?></p>
                    </div>
                    <div class="embed-responsive embed-responsive-16by9">
                      <img src="backend/foto/<?= $row['gambar']; ?>" alt="" class="embed-responsive-item rounded" style="object-fit: cover;">
                    </div>
                    ...
                    <div class="card-body">
                      ...
                      <p class="card-text line-clamp"><?= $row['isi_berita']; ?></p>
                      <a href="detailberita.php?id=<?= $row['id_berita']; ?>">Baca Detail <span class="iconify" data-icon="bi:arrow-right" style="color: #007bff; font-size: 16px;"></span></a>
                    </div>
                  </div>
                </div>
              <?php } ?>
